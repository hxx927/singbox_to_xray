...
安装后可以运行 `sudo s-x` 打开中文交互菜单，选择 `1` 查看入站，选择 `2` 执行推荐的安全预检。查看结果会结合当前 Xray 分成可迁移、已存在、冲突和不支持四组；S-UI 数据库仍会保留原入站，但完成迁移的同 tag 节点不会继续显示为可迁移。正式迁移、旧 client 吊销和回滚分别要求输入 `APPLY`、`REVOKE` 和 `ROLLBACK`，避免误操作。
...
每次正式迁移可以选择一个或多个入站。上一批必须先完成 REVOKE 或回滚，脚本才允许开始下一批，防止状态文件中的源 client 指纹和最近一次备份被覆盖。
