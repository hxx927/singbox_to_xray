...
仓库已提供 [`singbox_to_xray.py`](../singbox_to_xray.py)，可自动执行本文的字段转换、完整 Xray 配置合并、预检、备份、重启、Agent 上报、主控节点核验和回滚。

> 使用 `--notify-master` 前，主控必须升级到包含 `/api/remote/sync-nodes` 的版本。主控与脚本不匹配时，Xray 不会被擅自回滚，但脚本会报错并标记主控同步未确认。
...
```bash
sudo python3 singbox_to_xray.py deploy --apply --notify-master
```

成功时状态文件 `/var/lib/mmwx-singbox-migrate/state.json` 中的 `status` 为 `synced`；只到 `reported` 不代表节点已入库。
