...
安装后可以运行 `sudo s-x` 打开中文交互菜单，选择 `1` 查看入站，选择 `2` 执行推荐的安全预检。查看结果会结合当前 Xray 分成可迁移、已存在、冲突和不支持四组；S-UI 数据库仍会保留原入站，但完成迁移的同 tag 节点不会继续显示为可迁移。正式迁移和旧 client 吊销分别要求输入 `APPLY` 和 `REVOKE`，避免误操作。
...
预检和正式迁移会列出入站编号：可输入 `2`、`1,3`、`1-3` 或直接回车全选。菜单选择正式迁移后，脚本会询问是否自动停止占用目标端口的旧 Core；该选项只对全量迁移生效。部分迁移检测到端口占用时，脚本保留转换结果并等待用户只停用所选入站，输入 `CHECK` 后复查所选端口与未选在线端口；它不会停止整个服务。全量迁移输入 `y` 时，只会对 `sui`/`s-ui` 和 `sing-box` 执行对应的 `systemctl stop`。如果 Xray 部署失败，脚本会恢复备份并重新启动由它停止的来源服务。

每次正式迁移可以选择一个或多个入站。上一批必须先完成 REVOKE；手动撤销时，必须确认上一批 tag 已从当前 Xray 配置删除。否则脚本会拒绝开始下一批，防止状态文件中的源 client 指纹和最近一次备份被覆盖。
...
如果 sing-box 与 Xray 在同一台机器上，二者不能同时监听相同的 IP 和端口。因此可以做到“先验证、可手动撤销、客户端配置不变”，但最终抢占原端口时仍会有数秒切换窗口，不能承诺严格的零丢包。
...
1. 在 miaomiaowuX 删除刚转换的节点。
2. 确认 Agent 当前 Xray 配置中的同 tag 入站也已删除并重启 Xray，释放原端口；只删除主控列表并不足够。
3. 启动 S-UI/sing-box Core。
4. 确认原端口重新监听，再用旧客户端验证。
5. 执行“扫描远程服务 → 接受 Agent 现状”，清理主控与 Agent 的状态差异。
6. 对照握手参数查差异，不要直接重新生成 UUID、密码或 REALITY 密钥。

## 使用一键转换脚本

仓库已提供 [`singbox_to_xray.py`](../singbox_to_xray.py)，可自动执行本文的字段转换、完整 Xray 配置合并、预检、来源服务停止、备份、重启和 Agent 上报。脚本不提供显式回滚命令；部署失败时仍会自动恢复写入前的 Xray 备份。
...
吊销成功后脚本会直接列出相应 inbound 中剩余 client 的标签和完整凭据，无需再运行 `jq`；凭据属于敏感信息，请勿公开。找到管理员 client（例如 `黑西西`），复制其 credential 值；然后到“节点管理”打开同 tag 节点的 Clash 配置详情，只把 `uuid` 替换为管理员 UUID。不要删除 Xray 入站或删除节点后重新扫描，否则 Agent 会扫描不到该业务入站；保存节点后更新管理员订阅并验证真实连接。
